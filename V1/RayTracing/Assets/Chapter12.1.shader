...
Shader "Unlit/RayTracingSphere"
{
    Properties
    {
        ...
        _sphereRefraction1("Left Sphere Refraction", Range(0, 2.25)) = 1.5
    }
        ...
        Pass {
            ...
            float3 get_pixel_position(float2 uv, float2 sample_offset) {
                ...
                return _camPos - float3(
                    3.0 * half_width * (uv.x - 0.5 + sample_offset.x),
                    3.0 * half_height * (uv.y - 0.5 + sample_offset.y),
                    3.0 * half_depth
                );
            }
            ...
            float4 ray_color(float3 origin, float3 direction)
            {
                ...
                //bubble
                spheres[4].center = float3(_sphere1XAxis+1.0, _sphere1YAxis, _sphere1ZAxis - 1.0);
                spheres[4].radius = 0.35;
                spheres[4].material.type = 2;
                spheres[4].material.albedo = float3(0.8, 0.8, 0.8);
                spheres[4].material.refraction_index = 1.0/_sphereRefraction1;
