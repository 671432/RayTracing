...
Shader "Unlit/RayTracingSphere"
{
    Properties
    {
        ...
        _SamplesPerPixel ("Antialiasing (SPP)", Range(1, 3)) = 2
    ...
    SubShader {
        Pass {
            ...
            float3 scatter_metal(float3 incident, float3 normal) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        if (hit_sphere(spheres[i], origin, direction, ray_t, t, hitMaterial)) {
                            ...
                            if (hitMaterial.type == 0) {  // Lambertian
                                ...
                                direction = scatter_metal(direction, normal);
                            }
