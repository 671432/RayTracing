...
Shader "Unlit/RayTracingSphere"
{
    Properties
    {
        ...
        _FOV ("Field of View", Range(0.01, 179)) = 90
        _lookAt("Look At", Vector) = (0, 0, 0)
        _VUP("View Up", Vector) = (0, 0, 0)
        ...
        _sphereRefraction1("Left Sphere Refraction", Range(0, 1.0)) = 0.5
    }
    ...
    SubShader {
        Pass {
            ...
            void get_camera_basis_vector(float3 lookfrom, float3 _lookAt, float3 vup, out float3 u, out float3 v, out float3 w) {
                w = normalize(lookfrom - _lookAt); // Camera direction (pointing backwards)
                u = normalize(cross(vup, w)); // Right vector
                v = cross(w, u); // True "up" vector
            }
            ...
            float3 get_pixel_position(float2 uv, float2 sample_offset) {
                ...
                float3 u, v, w;
                get_camera_basis_vector(_camPos, _lookAt, _VUP, u, v, w);

                // Compute pixel position based on the new coordinates
                // return half_xyz because directly using _ViewportSize.xyz made issues
                float3 pixel_position = _camPos - w
                    + (3.0 * half_width * (uv.x - 0.5 + sample_offset.x)) * -u
                    + (3.0 * half_height * (uv.y - 0.5 + sample_offset.y)) * -v;
            ...
            float3 scatter_dielectric(float3 incident, float3 normal, float refraction_index, bool front_face) {
                ...
                r0 = r0 * r0;
                float reflect_prob = r0 + (1.0 - r0) * pow(1.0 - cos_theta, 5);

                float3 direction;

                if (cannot_refract || random_float(float2(0.0, 1.0)) < reflect_prob) {
                    // Reflect
            ...
            float3 scatter_metal(float3 incident, float3 normal, float fuzz) {
                ...
                //reflected += fuzz * random_on_hemisphere(normal);
            ...
            float4 ray_color(float3 origin, float3 direction)
            {
                ...
                spheres[1].material.albedo = float3(1, 1, 1);
                ...
                spheres[2].material.type = 2;
                spheres[2].material.albedo = float3(1, 1, 1);
                spheres[2].material.refraction_index = 1.0/_sphereRefraction1;
                //right
                spheres[3].center = float3(_sphere1XAxis-1.0, _sphere1YAxis, _sphere1ZAxis - 1.0);
                spheres[3].radius = 0.5;
                spheres[3].material.type = 1;
                spheres[3].material.albedo = float3(0.8, 0.6, 0.2);
                spheres[3].material.fuzz = _sphereFuzz2;
                //ground
                spheres[4].center = float3(_sphere2XAxis, 100.5 + _sphere2YAxis, _sphere2ZAxis - 1.0);
                spheres[4].radius = 100.0;
                spheres[4].material.type = 0;
                spheres[4].material.albedo = float3(0.8, 0.8, 0.0);
                ...
                for (int bounce = 0; bounce < _bounces; bounce++) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        if (hit_sphere(spheres[i], origin, direction, ray_t, t, hitMaterial)) {
                            ...
                            float3 outward_normal = normalize(hit_point - spheres[i].center);
                            float3 normal;
                            bool front_face;
                ...
                //return float4(pow(final_color * attenuation, _gamma), 1.0);
                float3 final = clamp(final_color * attenuation, 0.0, 1.0);
                return float4(pow(final, 1.0 / _gamma), 1.0);
